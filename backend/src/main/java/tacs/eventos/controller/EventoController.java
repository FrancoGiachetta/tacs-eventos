...
public class EventoController {
    ...
    public ResponseEntity<InscripcionResponse> getInscripcion(@AuthenticationPrincipal Usuario usuarioLogueado,
        ...
        return inscripcion.filter(i -> i.getEstado() != EstadoInscripcion.CANCELADA)
                ...
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "El usuario no está inscripto al evento"));
    }
    ...
    public ResponseEntity<List<InscripcionEnWaitlistResponse>> getWaitlistDeEvento(
        ...
        return ResponseEntity.ok(this.inscripcionesService.inscripcionesPendientes(evento).stream().map((InscripcionEvento i) -> {
            var usuarioResponse = new UsuarioResponse(i.getParticipante().getId(), i.getParticipante().getEmail(),
                    i.getParticipante().getRoles());
