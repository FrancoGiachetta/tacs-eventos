...
export default function EventCard({ evento, onVerDetalle }: EventCardProps) {
  const getEstadoChip = () => {
    if (!evento.abierto) return { text: 'Completo', color: 'bg-red-100 text-red-800' };
    if (evento.cupoMaximo === 0) return { text: 'Lista de espera', color: 'bg-yellow-100 text-yellow-800' };
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex gap-2">
            <span className={`px-2 py-1 rounded-full text-sm ${chip.color}`}>
              {chip.text}
            </span>
            {evento.categoria && (
