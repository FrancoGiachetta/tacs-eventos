...
function EventoRow(evento: Evento) {
    ...
    return <tr className="table-row even:bg-gray-100 odd:bg-gray-50">
        <td className="px-4 py-2">{evento.titulo}</td>
        <td className="px-4 py-2">{formatDate(new Date(evento.fechaHoraInicio), {withTime: true})}</td>
        <td className="px-4 py-2">{evento.ubicacion}</td>
        <td className="px-4 py-2">{evento.abierto ? "SÍ" : "No"}</td>
        <td className="px-4 py-2">{inscripciones.length}/{evento.cupoMaximo}</td>
        <td className="px-4 py-2">{waitlist.length}</td>
        <td className="px-4 py-2"><EventoActionMenu evento={evento} className="bg-transparent"/></td>
    </tr>
}